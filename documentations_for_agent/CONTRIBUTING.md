...
- Определение вебинара по `webinar_date` из payload
- Фильтрация (простая и комбинированная AND/OR)
- Модульная система (регистрация, роуты, виджеты)
...
Тесты используют отдельную БД (`ecomspace_analytics_test`). Перед каждым тест-сьютом БД сбрасывается и накатываются миграции. Это настраивается в `packages/api/src/__tests__/setup.ts`.
